$(function (){
    ...
    let tbl_exposiciones = $('#table-exposiciones');
    if(tbl_exposiciones.length){
        ...
        let dt_exposiciones = Cracknd.Datatables.render(tbl_exposiciones, '/administrador/exposiciones/get-exposiciones', Cracknd.Datatables.options(), null, columns);

        body.on('click', '.btn-drop-exposicion', function () {
            let id = $(this).data('id');
            Cracknd.Ajax.SweetAlert('delete', {
                title: "¿Desea eliminar la exposición digital seleccionada?",
                button_color: "#dc3545",
                button_text: "Sí, eliminar"
            }, '/administrador/drop-exposicion', { id: id }, function (response) {
            ...
            let pond = FilePond.create(inputElement, {
                ...
                server: {
                    ...
                    process: {
                        ...
                        ondata: (formData) => {
                            formData.append('object_type', 'App\\ExposicionesDigitales');
        ...
        $('#btn-actualizar-exposicion-digital').click(function (){
            editor.save().then((outputData) => {
                ...
                Cracknd.Ajax.SweetAlert('update', {
                    title: "¿Desea actualizar la exposición digital?",
                    button_color: "#fcce54",
                    button_text: "Sí, actualizar"
                 }, '/administrador/update-exposicion', form.serialize() + `&${form_cover.serialize()}&descripcion=${contenido_markup}`, function (data) {
            ...
            let pond = FilePond.create(inputElement, {
                ...
                server: {
                    ...
                    process: {
                        ...
                        ondata: (formData) => {
                            formData.append('object_type', 'App\\ExposicionesDigitalesMedia');
    ...
    //btn-confirmar-exposicion
        ...
        $('#btn-confirmar-exposicion').click(function (){
            let form = $('#registro-modal-agregar');
            Cracknd.Ajax.SweetAlert('add', {
                title: "¿Desea agregar la Exposición Digital?",
                button_color: "#6bad47",
                button_text: "Sí, agregar"
            }, '/administrador/add-exposicion', form.serialize(), function (response) {
                if(response.status){
                    let id = response.data;
                    window.location = BASEURL + '/administrador/exposiciones/editar/' + id;
                } else
                    swal({"html": true, "title": "Espera, ¡algo ocurrió!", "text": response.message, "type":"error"});
            });
        });
