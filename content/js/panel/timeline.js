$(function (){
    ...
    $('#museografia-crear').click(function(){
        var titulo = $('#modal-agregar-nombre').val();
        $('#museografia-create').imgPicker({
            url: baseurl+'/app/libs/upload/upload_museografia.php',
            aspectRatio: 1/1,
            uploadSuccess: function(image) {
                proporcion = (image.height > image.width) ? 2/3 : 16/9;
                ...
                this.options.aspectRatio = proporcion;
                this.options.setSelect = select;
            },
            cropSuccess: function(image) {
                $('.vista-previa-museografia').append('<div class="col-sm-3">' +
                                                        '<img src="'+baseurl+'public/img/museografia/'+image.versions.tmb.url+'" class="img-thumbnail img-responsive">' +
                                                        '<a href="#" class="btn btn-danger btn-block btn-drop-museografia btn-xs"><i class="fa fa-times"></i> Eliminar</a>' +
                                                        '<input type="hidden" name="museografia_original[]" value="'+image.versions.museografia.url+'">' +
                                                        '<input type="hidden" name="museografia_mobile[]" value="'+image.versions.tmb.url+'">' +
                                                    '</div>');
            },
            data:{
                titulo: titulo
