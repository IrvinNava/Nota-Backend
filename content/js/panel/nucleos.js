$(function (){
    ...
    $('#btn-confirmar-nucleo').click(function (){
            let form = $('#registro-modal-agregar');
            Cracknd.Ajax.SweetAlert('add', {
                title: "¿Desea agregar el núcleo?",
                button_color: "#6bad47",
                button_text: "Sí, agregar"
            }, '/administrador/add-nucleo', form.serialize(), function (response) {
                if(response.status){
                    let id = response.data;
                    window.location = BASEURL + '/administrador/exposiciones/nucleo/editar/' + id;
                } else
    ...
    let tbl_exposiciones = $('#table-nucleos-exposciones-digitales');
    if(tbl_exposiciones.length){
        ...
        let dt_exposiciones = Cracknd.Datatables.render(tbl_exposiciones, '/administrador/nucleos/get/'+id, Cracknd.Datatables.options(), null, columns);

        body.on('click', '.btn-drop-exposicion', function () {
            let id = $(this).data('id');
            Cracknd.Ajax.SweetAlert('delete', {
                title: "¿Desea eliminar el núcleo seleccionado?",
                button_color: "#dc3545",
                button_text: "Sí, eliminar"
            }, '/administrador/drop-nucleo', { id: id }, function (response) {
    ...
    if(form.length){
            ...
            let pond = FilePond.create(inputElement, {
                ...
                server: {
                    ...
                    process: {
                        ...
                        ondata: (formData) => {
                            formData.append('object_type', 'App\\NucleosExposicionesDigitales');
                ...
                image: {
                    ...
                    config: {
                        ...
                        additionalRequestData: {
                            object_type: 'App\\NucleosExposicionesDigitales',
                            object_id: 1,
                            uploader: 'editorjs'
                        },
                        field: 'upload_file',
        ...
        $('#btn-actualizar-exposicion-digital').click(function (){
            editor.save().then((outputData) => {
                let form = $('#form-registro');
                let id_exposicion = $('#id_exposicion').val();
                ...
                Cracknd.Ajax.SweetAlert('update', {
                    title: "¿Desea actualizar el núcleo?",
                    button_color: "#fcce54",
                    button_text: "Sí, actualizar"
                 }, '/administrador/update-nucleo', form.serialize() + `&${form_cover.serialize()}&descripcion=${contenido_markup}&id_exposicion=${id_exposicion}`, function (data) {
                    window.location = BASEURL + '/administrador/exposiciones/nucleos/'+id_exposicion;
                });
            }).catch((error) => {
            ...
            let pond = FilePond.create(inputElement, {
                ...
                server: {
                    ...
                    process: {
                        ...
                        ondata: (formData) => {
                            formData.append('object_type', 'App\\NucleosExposicionesDigitalesMedia');
